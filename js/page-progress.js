/* Global Deutschsprechen lesson progress */
...
function ratio(text){var m=(text||"").match(/(\d+)\s*\/\s*(\d+)/);return m&&+m[2]?Math.max(0,Math.min(100,+m[1]/+m[2]*100)):null}
function activityPct(){
 var c=document.getElementById("progress-count");if(c){var x=ratio(c.textContent);if(x!==null)return x}
 var s=document.getElementById("stat-progress");if(s){var y=ratio(s.textContent);if(y!==null)return y}
